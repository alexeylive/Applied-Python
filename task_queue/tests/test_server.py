...
#Перед запуском убедитесь, чтобы в директории, где лежит server.py не было cash (сохраненных данных)
...
class ServerBaseTest(TestCase):
    ...
    def test_wrong_data(self):
        self.assertEqual(b'ERROR', self.send(b'ADD 1 6 12345'))

    def test_save(self):
        global id
        id = self.send(b'ADD 5 5 12345')
        self.assertEqual(b'YES', self.send(b'IN 5 ' + id))
        self.assertEqual(b'OK', self.send(b'SAVE'))

    def test_upload_saved(self):
        self.assertEqual(b'YES', self.send(b'IN 5 ' + id))
        self.assertEqual(b'NO', self.send(b'IN 5 1234'))
